...
class MQTTClient {
  ...
  sendState2Client(topic, state, qos, dup, retain, cb) {
    ...
    let smsg = {
      ...
      func: () => { this.client.publish({ topic: topic, payload: state, qos: qos, dup: dup, retain: retain, messageId: this.messageId }, cb); }
    };
      ...
      for (let i in all) {
        if (id.startsWith(i + '-')) {
          ...
          for (let j in devices) {
            ...
            let state = devices[j];
            state.state = j;
            let deviceid = state.devicename || this.getId();
            ...
            if (state.http && state.http.publish) {
  ...
  createIoBrokerState(topic, payload) {
    ...
    for (let i in dps) {
      let dp = dps[i];
      let deviceid = dp.devicename || this.getId();
  ...
  listen() {
    ...
    this.client.on('connect', (packet) => {
      ...
      if (packet.username === this.adapter.config.user && packet.password.toString() === this.adapter.config.password) {
        ...
        this.adapter.log.debug('User: ' + packet.username + ', Password: ' + packet.password.toString());
