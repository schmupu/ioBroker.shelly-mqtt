...
function MQTTServer(adapter, states) {
    ...
    function updateClients() {
        ...
        adapter.log.info('UpdateClients: info.connection' + text);
    ...
    function processTopic(id, topic, message, qos, retain, isAck, obj, ignoreClient, cb) {
        ...
        if (obj && namespaceRegEx.test(id) && (!obj.native || !obj.native.topic)) {
            ...
            adapter.log.info("processTopic adapter.setForeignObject");
            ...
            if (!obj) {
                adapter.getForeignObject(id, (err, obj) => {
                    if (!obj) {
                        ...
                        adapter.log.info("adapter.setForeignObject " +id);
                        /*
    ...
    (function _constructor(config) {
        ...
        adapter.getObject('info.connection', (err, obj) => {
            if (!obj || !obj.common || obj.common.type !== 'string') {
                ...
                adapter.log.info("info.connection");
        ...
        config.port = parseInt(config.port, 10) || 1882;
        config.bind = config.bind || '0.0.0.0';
